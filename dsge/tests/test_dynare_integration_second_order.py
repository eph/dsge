...
"""
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        endo_names = _matlab_char_array_to_strings(_get_struct_field(M_, "endo_names"))
        ...
        state_names = [endo_names[i - 1] for i in state_var]  # Dynare uses 1-based indices
        nendo = len(endo_names)
        nstate = len(state_var)
        nexo = len(exo_names)

        # Reorder ours to Dynare's (endo, state) ordering.
        ours_endo_idx = [ours["endo_names"].index(n) for n in endo_names]
