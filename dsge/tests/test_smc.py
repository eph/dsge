...
def test_smc_smoke_nkmp_model():
    ...
    with warnings.catch_warnings():
        warnings.filterwarnings("ignore", message=".*unstable roots.*", category=UserWarning)
        warnings.filterwarnings("ignore", message="Indeterminacy: multiple stable solutions satisfy the model.", category=UserWarning)
        res = m.estimate_smc(
            order=1,
            backend="local",
            smc_options={
